...
- First layer embedding "will take each observation ... and make dense vectors from our word sequences. This turns each observation into a ... data cube."
- "The [flatten] layer takes the matrix for each observation and flattens them into one dimension. This [creates] a ... long vector for each observation."
...
require(viridis)
tibble(
  epoch = seq_len(length(dnn_fit$metrics$loss)),
  loss = dnn_fit$metrics$loss,
  val_loss = dnn_fit$metrics$val_loss,
  accuracy = dnn_fit$metrics$accuracy,
  val_accuracy = dnn_fit$metrics$val_accuracy
) |>
  pivot_longer(
    cols = -epoch,
    names_to = "metric",
...
```{r roc_compare, fig.height = 5, fig.width = 5}
require(tidyverse)
require(tidymodels)
tidymodels_prefer()
ref_roc <-
